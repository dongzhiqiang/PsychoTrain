...
public class CombatPart : RolePart
{
    ...
    public bool IsPlaying { get { return StatePart.CurStateType == enRoleState.combat; } }
    ...
    public WeaponCfg FightWeapon
    {
        ...
        set
        {
            ...
            //强制切换动作
            if (StatePart.GotoState(enRoleState.switchWeapon))
            {
                m_fightWeapon = value;
    ...
    {
        m_combatState = StatePart.StateCombat;
    ...
    {
        //自动朝向
        Vector3 lastDir = StatePart.StateMove.CurDir;
        SkillCfg cfg = s.Cfg;
    ...
    public Skill GetSkill(enSkillType skillType)
    {
        ...
        string skillId = null;
        if (m_fightWeapon != null)
            skillId = m_fightWeapon.GetSkillId(skillType, StatePart.IsAir);
        ...
        skillId = roleCfg.GetSkillId(skillType, StatePart.IsAir);
        if (!string.IsNullOrEmpty(skillId))
            return GetSkill(skillId);
    ...
    public enPlaySkill CanPlay(Skill s, bool cancelAndCombo = true)
    {
        ...
        if (StatePart.IsSilent) return enPlaySkill.fail;

        if (StatePart.IsAir && !s.Cfg.isAirSkill) return enPlaySkill.fail;//在空中，但是不是空中技能
