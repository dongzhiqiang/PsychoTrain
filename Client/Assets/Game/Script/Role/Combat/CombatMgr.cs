...
public class CombatMgr : SingletonMonoBehaviour<CombatMgr>
{
    ...
    //isCut表明要不要做生命偷取计算
    public bool Damage(Role source, Role target, Vector3 pos, float rate, string hitFx, bool isCut)
    {
        ...
        return Damage(source, target, pos, (int)damage, hitFx, isCritical, isCut, hitDef);
    }

    //pos表明了打击的位置,cutHp表明扣的血量,hitFx表明打击特效
    public bool Damage(Role source, Role target, Vector3 pos, int cutHp, string hitFx, bool isCritical, bool isCut, enHitDef hitDef)
    {
        if (source.State != Role.enState.alive || target.State != Role.enState.alive)
        {
            Debuger.LogError("角色已经死亡 不能伤害计算:{0} {1}", source.Cfg.id, target.Cfg.id);
        ...
        //先扣血
        int trueCut = AddHp(target, -cutHp, isCritical, true, hitDef);
        ...
        if (!string.IsNullOrEmpty(hitFx))
            RoleFxCfg.Play(hitFx, target, source, pos);
    ...
    //增减血，返回实际加或者减了多少
    public int AddHp(Role target, int add, bool isCritical, bool showUI, enHitDef hitDef = enHitDef.none)
    {
        if (add == 0)
            return 0;
