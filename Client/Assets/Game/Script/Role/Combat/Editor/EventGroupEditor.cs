...
public class EventGroupEditor : MultiWindow
{
    ...
    public bool DrawFrameHeader(ref Rect r, SkillEventFrameCfg frameCfg, SkillEventGroupCfg g, int col, System.Action<string> onTip)
    {
        switch ((enCol)col)
        {
            ...
            case enCol.ingore:
                if (HeaderButton(ref r, EditorGUIUtility.IconContent("ViewToolOrbit On"), COL_WIDTH))
                {
                    if (g.frames.Count == 0)
                        return false;
            ...
            case enCol.frameType:
                if (HeaderButton(ref r, "帧类型", COL_WIDTH * 2)) onTip(
@"单帧
多帧，允许设置间隔
...
"); return false;
            ...
            case enCol.targetType:
                if (HeaderButton(ref r, "对象", COL_WIDTH * 5)) onTip(
@"作用对象类型:
    释放者，总是执行到，除非角色不可战斗了
    释放者，受碰撞检测
    友方，同一阵营但是不是自己
...
"); return false;
            case enCol.range:
                if (HeaderButton(ref r, "范围", COL_WIDTH * 8)) onTip(
@"作用范围类型:
    圆。参数(半径、垂直距离)
    扇形。参数(半径、夹角限制、垂直距离)
    矩形。参数(长、宽、垂直距离)
...
垂直距离，-1表示不用判断了"); return false;
            ...
            case enCol.targetOrderType:
                if (HeaderButton(ref r, "排序", COL_WIDTH * 2)) onTip(
@"
无，一般是按照创建顺序
洗牌
距离
