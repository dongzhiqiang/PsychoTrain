...
public class RoleFxCfg
{
    ...
    public static GameObject Play(string name, Role source, Role target, Vector3 pos)
    {
        RoleFxCfg cfg = Get(name);
        if (cfg == null) return null;

        return Play(cfg, source, target, pos, true);
    }
    ...
    public static GameObject Play(int id, Role source, Role target, bool checkDelayDestroy = true)
    {
        RoleFxCfg cfg = Get(id);
        if (cfg == null) return null;
        return Play(cfg, source, target, source.TranPart.GetYOff(0.5f), checkDelayDestroy);
    }

    public static GameObject Play(RoleFxCfg cfg, Role source, Role target, Vector3 pos, bool checkDelayDestroy)
    {
        if (cfg == null) return null;
        ...
        GameObject go = fxCreateCfg.Create(source, target, pos);
        if (go == null)
            return null;
