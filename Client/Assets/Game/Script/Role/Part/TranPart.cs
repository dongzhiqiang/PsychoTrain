...
public class TranPart : RolePart
{
    ...
    {//这里可以认为是模型的位置，用于伤害计算
        ...
        {
            //如果在浮空或者击翻状态下要调整下位置
            var behit = StatePart.StateBehit;
    ...
    void Sample()
    {
        ...
        {
            //位置
            if (!StatePart.IsLimitMove && !StatePart.IsNoCollider)
            {
            ...
            //模型y轴位移
            if (m_tranModel != null && RoleModel.IsShow && !StatePart.IsNoCollider)
            {
                //y存在的话，重力已经计算在上面做了，否则在这里做下
                if (y == Vector3.zero && !IsGrounded && !StatePart.IsAir)
                {
