...
public class RoleStateAniCxt
{
    ...
    public static RoleStateAniCxt Parse(string anisParam, string avoidParam)
    {
        ...
        if (!StatePart.TryParse(avoidParam, ref cxt.avoidRoleStates))
            return null;
...
public class RoleStateAni : RoleState
{
    ...
    #region Frame
    public RoleStateAni(StatePart rsm, enRoleState enterType)
    : base(rsm, enterType)
    {
    ...
    public bool Goto(RoleStateAniCxt cxt, bool force = false)
    {
        ...
        if (!force && cxt != null)
            force = cxt.avoidRoleStates.Contains(StatePart.CurStateType);
        return m_statePart.GotoState(Type, cxt, force);//如果这个上下文免疫了运行中状态，那么强制进入本状态
    }
    ...
    public bool IsAvoidState(enRoleState st)
    {
        if (!IsPlaying)
        {
            Debuger.LogError("逻辑错误，调用这个接口前要先判断下m_statePart的IsAnis为true才能调用");
            return false;
        }
