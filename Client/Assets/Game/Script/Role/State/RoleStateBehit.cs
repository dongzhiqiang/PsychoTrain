...
public class RoleStateBehit : RoleState
{
    ...
    public RoleStateBehit(StatePart rsm, enRoleState enterType)
    : base(rsm, enterType)
    {
    ...
    public override void Enter(object param)
    {
        ...
        //如果在空中被击，那么一律进入下落状态
        bool isAir = StatePart.IsAir;
        if (isAir) StatePart.IsAir = false;//退出空中状态
    ...
    public override void Update()
    {
        ...
        {
            //死亡
            if (StatePart.CheckDead(false))
                return;
            //移动中切移动
            else if (StatePart.CheckMove())
                return;
            //待机
            else if (StatePart.CheckFree())
                return;
        }
