...
public class BornEffectEditor : EditorWindow
{
    ...
    void DrawInfo()
    {
        ...
        if (GUILayout.Button("预览出生方式"))
        {
            ...
            if (role != null && role.StatePart != null && (role.StatePart.CurStateType == enRoleState.born || role.StatePart.CurStateType == enRoleState.dead))
            {
            ...
            role.StatePart.GotoState(enRoleState.born, new RoleStateBornCxt(m_BornCfg.typeName));
            roleID = role.Id;
        }
        ...
        using (new AutoBeginHorizontal())
        {
            if (GUILayout.Button("预览站立死亡方式"))
            {
                ...
                role.StatePart.GotoState(enRoleState.dead, new RoleStateDeadCxt(m_DeadCfg.typeName));
            }
    ...
    IEnumerator playGroundDead(Role role)
    {
        ...
        while (role.AniPart.CurSt.normalizedTime < 1f)
            yield return 0;
        role.StatePart.GotoState(enRoleState.dead, new RoleStateDeadCxt(m_GroundDeadCfg.typeName));
