...
public class BornLogic
{
    ...
    public void Update()
    {
        if (m_bornCfg != null)
        {
            ...
            {
                if (m_role.StatePart.CurStateType == enRoleState.born)
                    m_role.StatePart.CheckFree(true);
                else if (m_role.StatePart.CurStateType == enRoleState.dead)
                {
                    //RoleMgr.instance.DestroyRole(m_role);
                }
                //AniPart aniPart = m_role.StatePart.AniPart;
    ...
    public bool isEnd()
    {
        if (m_runTime < m_bornCfg.aniDelay || (m_aniCxt != null && !m_role.StatePart.AniPart.IsAnisOver(m_aniCxt)))   //动作不用时间来判断了
            return false;
